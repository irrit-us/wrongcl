...
use wrongcl_native::endpoint::{
    Endpoint, KcpOptions, OuterSecurity, ProxyProtocol, Transport, VlessOptions,
};
...
#[test]
fn probe_works_against_kcp_server() {
    ...
    let client = WrongsvClient::new(ServerConfig {
        ...
        endpoint: Endpoint {
            ...
            transport: Transport::Kcp(KcpOptions {
                seed: TEST_SEED.into(),
                mtu: 1400,
                tti: 20,
            }),
    ...
    let result = client
        ...
        .expect("probe over KCP");
    assert_eq!(result.preview, "ping-kcp");
}

#[test]
fn socks_proxy_works_against_kcp_server() {
    let server = spawn_kcp_server();
    let echo_addr = spawn_tcp_echo_server();
    ...
    let response = run_socks_echo(proxy.snapshot().socket_addr(), echo_addr, b"hello-kcp").unwrap();
    proxy.stop().unwrap();

    assert_eq!(response, b"hello-kcp".to_vec());
}
