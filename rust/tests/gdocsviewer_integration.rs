...
use wrongcl_native::endpoint::{
    Endpoint, GdocsViewerOptions, OuterSecurity, ProxyProtocol, Transport, VlessOptions,
};
...
const TEST_SHARED_KEY: &str = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=";

struct GdocsViewerServer {
    port: u16,
    _shutdown: ShutdownSignal,
    _handle: wrongsv_server::ServerHandle,
}

fn spawn_gdocsviewer_server(shared_key: Option<&str>) -> GdocsViewerServer {
    let port = free_tcp_port();
    let shared_key = shared_key
        .map(|value| format!("shared_key = \"{value}\"\n"))
        .unwrap_or_default();
...
#[test]
fn probe_works_against_gdocsviewer_server() {
    ...
    let client = WrongsvClient::new(ServerConfig {
        ...
        endpoint: Endpoint {
            ...
            transport: Transport::Gdocsviewer(GdocsViewerOptions {
                path_prefix: "/gdocsviewer".into(),
                shared_key: None,
            }),
    ...
    let result = client
        ...
        .expect("probe over gdocsviewer");
    assert_eq!(result.preview, "ping-gdocs");
}

#[test]
fn socks_proxy_works_against_gdocsviewer_server() {
    let server = spawn_gdocsviewer_server(None);
    let echo_addr = spawn_tcp_echo_server();
    ...
    let response =
        run_socks_echo(proxy.snapshot().socket_addr(), echo_addr, b"hello-gdocs").unwrap();
    proxy.stop().unwrap();

    assert_eq!(response, b"hello-gdocs".to_vec());
}

#[test]
fn socks_proxy_udp_works_against_gdocsviewer_server() {
    let server = spawn_gdocsviewer_server(None);
    ...
    let mut session = client
        ...
        .unwrap();
    session.send_packet(b"ping-gdocs-udp").unwrap();
    for _ in 0..800 {
        if let Some(packet) = session.try_recv_packet().unwrap() {
            assert_eq!(packet.payload, b"ping-gdocs-udp");
            return;
        }
        thread::sleep(Duration::from_millis(25));
    }
    panic!("no UDP response from gdocsviewer session");
}

#[test]
fn encrypted_probe_works_against_gdocsviewer_server() {
    ...
    let client = WrongsvClient::new(ServerConfig {
        ...
        endpoint: Endpoint {
            ...
            transport: Transport::Gdocsviewer(GdocsViewerOptions {
                path_prefix: "/gdocsviewer".into(),
                shared_key: Some(TEST_SHARED_KEY.into()),
            }),
            outer_security: OuterSecurity::None,
        },
    })
    ...
    let result = client
        .probe(
            &Target::new(echo_addr.ip().to_string(), echo_addr.port()).unwrap(),
            "ping-gdocs-encrypted",
        )
        .expect("probe over encrypted gdocsviewer");
    assert_eq!(result.preview, "ping-gdocs-encrypted");
}

#[test]
fn encrypted_socks_proxy_works_against_gdocsviewer_server() {
    let server = spawn_gdocsviewer_server(Some(TEST_SHARED_KEY));
    let echo_addr = spawn_tcp_echo_server();
    ...
    let response = run_socks_echo(
        ...
        echo_addr,
        b"hello-gdocs-encrypted",
    )
    .unwrap();
    proxy.stop().unwrap();

    assert_eq!(response, b"hello-gdocs-encrypted".to_vec());
}
