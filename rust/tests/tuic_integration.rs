...
use wrongcl_native::endpoint::{Endpoint, OuterSecurity, ProxyProtocol, Transport, TuicOptions};
...
const TEST_PASSWORD: &str = "tuic-pass";
...
fn spawn_tuic_server() -> TuicServer {
...
#[test]
fn probe_works_against_tuic_server() {
    let server = spawn_tuic_server();
    ...
    let client = WrongsvClient::new(ServerConfig {
        ...
        port: server.port,
        endpoint: Endpoint {
            proxy: ProxyProtocol::Tuic(TuicOptions {
    ...
    let result = client
        ...
        .expect("probe over TUIC");
    assert_eq!(result.preview, "ping-tuic");
}

#[test]
fn socks_proxy_works_against_tuic_server() {
    let server = spawn_tuic_server();
    let echo_addr = spawn_tcp_echo_server();
    ...
    let response =
        run_socks_echo(proxy.snapshot().socket_addr(), echo_addr, b"hello-tuic").unwrap();
    proxy.stop().unwrap();

    assert_eq!(response, b"hello-tuic".to_vec());
}

#[test]
fn socks_proxy_udp_works_against_tuic_server() {
    let server = spawn_tuic_server();
    ...
    panic!("no UDP response from TUIC session");
}
